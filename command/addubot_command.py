...
<blockquote><b>🔥 {bot.me.mention} Berhasil Di Aktifkan
 • Akun: <a href=tg://openmessage?user_id={kn_client.me.id}>{kn_client.me.first_name} {kn_client.me.last_name or ''}</a>
 • ID: <code>{kn_client.me.id}</code>
 • Plan: <b>{PLAN}</b>
 • Prefixes: {' '.join(prefix)}
 • Token: <code>{user_token}</code>
 • Masa Aktif: {expir}</b></blockquote>
...
Mohon simpan Token kamu dengan aman.</b></blockquote>"""
        ...
        return await client.send_message(LOG_SELLER, f"""<b>❏ Notifikasi Userbot Aktif</b>
